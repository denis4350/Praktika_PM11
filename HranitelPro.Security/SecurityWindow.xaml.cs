...
namespace HranitelPro.Security
{
    public partial class SecurityWindow : Window
    {
        ...
        public SecurityWindow(int empId, string empName)
        {
            InitializeComponent();
            employeeId = empId;
            ...
            try
            {
                ...
                using (var conn = new NpgsqlConnection(connectionString))
                {
                    ...
                    using (var cmd = new NpgsqlCommand(query, conn))
                    {
                        ...
                        {
                            var requests = new List<SecurityRequestItem>();
                            while (reader.Read())
                            {
                                var item = new SecurityRequestItem
                                {
                                ...
                                {
                                    if (!item.VisitorName.ToLower().Contains(searchText.ToLower()) &&
                                        !item.Passport.Contains(searchText))
                                        continue;
                                }
        ...
        {
            if (RequestsGrid.SelectedItem is SecurityRequestItem selectedRequest)
            {
                var accessWindow = new SecurityAccessWindow(selectedRequest, employeeId, employeeName);
    ...
    public class SecurityRequestItem
    {
